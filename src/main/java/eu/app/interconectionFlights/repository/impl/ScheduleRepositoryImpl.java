...
public class ScheduleRepositoryImpl implements ScheduleRepository {
	...
	public Schedule get(String from, String to, int month, int year) {
		URI uri = null;
		StringBuilder fullURL = new StringBuilder().append(APIRoutesUrl).append("/").append(from).append("/").append(to)
				.append("/years/").append(year).append("/months/").append(month);
		log.info(String.format("Looking for schedule flights on API %s", fullURL));
		try {
	...
	private Schedule getSchedule(URI uri, String from, String to, int month, int year) {
		...
		try {
			...
			if (response.getStatusCode().is2xxSuccessful()) {
				log.info(String.format("We have schedule flights from %s to %s on %d/%d", from, to, month, year));
				return response.getBody();
			}
		} catch (final HttpClientErrorException e) {
			log.error(String.format("We have not flights from %s to %s on %d/%d, StatusCode %s, Response %s", from, to,
					month, year, e.getStatusCode(), e.getResponseBodyAsString()));
		}
		return null;
	}
